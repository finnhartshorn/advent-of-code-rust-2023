pub fn part_one(input: &str) -> Option<u32> {
    Some(input.lines().map(
        |line| {
            let mut iter = line.split(':').nth(1).unwrap().split('|');
            let win = iter.next().unwrap().split_whitespace().map(|x| x.parse::<u32>().unwrap()).collect::<Vec<u32>>();
            let have = iter.next().unwrap().split_whitespace().map(|x| x.parse::<u32>().unwrap()).collect::<Vec<u32>>();
            let points: u32 = have.iter().map(|x| if win.contains(x) { 1 } else { 0 }).sum();
            if points == 0 {
                return 0
            }
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    Some(input.lines().map(
        |line| {
            ...
            let card_num = iter.next().unwrap().split_whitespace().nth(1).unwrap().parse::<u32>().unwrap();
